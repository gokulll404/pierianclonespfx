...
export const getNewJoiners = async (sp: SPFI, listName: string) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.select("Id", "Title","Email", "Designation", "Department", "UserImage", "JoiningDate","EmployeeID","Status").orderBy("Id", false)();
    } catch (err) {
        handleError(err, `getNewJoinersData for ${listName}`)
    }
}

export const addNewJoinee = async (sp: SPFI, listName: string, data: any) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.add(data);
    } catch (err) {
        handleError(err, `addNewJoinee to ${listName}`)
    }
}

export const updateNewJoinee = async (sp: SPFI, listName: string, itemId: number, updatedData: any) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.getById(itemId).update(updatedData);
    } catch (err) {
        handleError(err, `updateNewJoinee in ${listName} for ID ${itemId}`)
    }
}

export const deleteNewJoinee = async (sp: SPFI, listName: string, itemId: number) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.getById(itemId).delete();
    } catch (err) {
        handleError(err, `deleteNewJoinee in ${listName} for ID ${itemId}`)
    }
}
...
export const uploadFileToPictureLibraryAsync = async (
  ...
  try {
    ...
    const uploadResult = await sp.web
      .getFolderByServerRelativePath(`/sites/${Site_Name}/NewJoineeImages`)
