...
export const getCorporateEvents = async (sp: SPFI, listName: string) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.select("Id", "Title","Description", "Date", "Body", "Image", "Location","Status").orderBy("Date", false)();
    } catch (err) {
        handleError(err, `getCorporateEventsData for ${listName}`)
    }
}

export const addCoroperateEvent = async (sp: SPFI, listName: string, data: any) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.add(data);
    } catch (err) {
        handleError(err, `addCorporateEvent to ${listName}`)
    }
}

export const updateCoroperateEvent = async (sp: SPFI, listName: string, itemId: number, updatedData: any) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.getById(itemId).update(updatedData);
    } catch (err) {
        handleError(err, `updateCorporateEvent in ${listName} for ID ${itemId}`)
    }
}

export const deleteCoroperateEvent = async (sp: SPFI, listName: string, itemId: number) => {
    try {
        return await sp.web.lists.getByTitle(listName).items.getById(itemId).delete();
    } catch (err) {
        handleError(err, `deleteCorporateEvent in ${listName} for ID ${itemId}`)
    }
}
...
export const uploadFileToPictureLibraryAsync = async (
  ...
  try {
    ...
    const uploadResult = await sp.web
      .getFolderByServerRelativePath(`/sites/${Site_Name}/CorporateEventImages`)
